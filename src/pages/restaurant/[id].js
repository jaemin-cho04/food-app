// src/pages/restaurant/[id].js
import { useEffect, useState } from 'react';
import { useRouter } from 'next/router';
import Link from 'next/link';
import { supabase } from '../../lib/db'; // Connect to DB

export default function RestaurantProfile() {
  const router = useRouter();
  const { id } = router.query; // Grabs the ID from the URL (e.g., "1")
  const [restaurant, setRestaurant] = useState(null);
  const [loading, setLoading] = useState(true);

  // When the ID is ready, fetch the data
  useEffect(() => {
    if (id) fetchRestaurant();
  }, [id]);

  async function fetchRestaurant() {
    // Ask Supabase for ONE restaurant where the ID matches
  ...
  // Calculate Net Likes for display
  ...
  return (
    <div className="min-h-screen bg-white pb-10">
      ...
      <div className="p-6 max-w-md mx-auto">
        ...
        {/* Static placeholder menu for now */}
